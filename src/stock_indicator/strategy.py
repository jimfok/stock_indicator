...
def compute_signals_for_date(
    ...
    *,
    ...
    use_unshifted_signals:
        When ``True``, look for signal columns with the ``_unshifted`` suffix
        generated by strategy functions to evaluate signals on the same day
        they are produced.
    ...
    for csv_file_path, price_data_frame, symbol_mask in selected_symbol_data:
        ...
        for buy_name in buy_choice_names:
            ...
            if use_unshifted_signals:
                column_name = f"{buy_name}_entry_signal_unshifted"
        ...
        for sell_name in sell_choice_names:
            ...
            if use_unshifted_signals:
                column_name = f"{sell_name}_exit_signal_unshifted"
                if column_name in price_data_frame.columns:
...
def attach_ema_sma_cross_signals(
    ...
    if require_close_above_long_term_sma:
        ...
        price_data_frame["ema_sma_cross_entry_signal_unshifted"] = (
            ema_cross_up
            & (
        ...
        price_data_frame["ema_sma_cross_entry_signal_unshifted"] = ema_cross_up
    price_data_frame["ema_sma_cross_exit_signal"] = ema_cross_down.shift(
        1, fill_value=False
    )
    price_data_frame["ema_sma_cross_exit_signal_unshifted"] = ema_cross_down
...
def attach_20_50_sma_cross_signals(
    ...
    price_data_frame["20_50_sma_cross_entry_signal_unshifted"] = (
        sma_20_crosses_above_sma_50
    )
    price_data_frame["20_50_sma_cross_exit_signal_unshifted"] = (
        sma_20_crosses_below_sma_50
    )
...
def attach_ema_sma_cross_with_slope_signals(
    ...
    price_data_frame["ema_sma_cross_with_slope_entry_signal_unshifted"] = (
        price_data_frame["ema_sma_cross_entry_signal_unshifted"]
        & (price_data_frame["sma_angle"] >= angle_lower_bound)
        & (price_data_frame["sma_angle"] <= angle_upper_bound)
    )
    price_data_frame["ema_sma_cross_with_slope_exit_signal_unshifted"] = (
        price_data_frame["ema_sma_cross_exit_signal_unshifted"]
    )
...
def attach_ema_sma_cross_testing_signals(
    ...
    price_data_frame["ema_sma_cross_testing_entry_signal_unshifted"] = (
        price_data_frame["ema_sma_cross_entry_signal_unshifted"]
        & (price_data_frame["sma_angle"] >= angle_lower_bound)
        & (price_data_frame["sma_angle"] <= angle_upper_bound)
        & near_ok.fillna(False)
    ...
    price_data_frame["ema_sma_cross_testing_exit_signal_unshifted"] = (
        price_data_frame["ema_sma_cross_exit_signal_unshifted"]
    )
...
def attach_ema_shift_cross_with_slope_signals(
    ...
    price_data_frame["ema_shift_cross_with_slope_entry_signal_unshifted"] = (
        crosses_up
        & (price_data_frame["shifted_ema_angle"] >= angle_lower_bound)
        & (price_data_frame["shifted_ema_angle"] <= angle_upper_bound)
    )
    price_data_frame["ema_shift_cross_with_slope_exit_signal_unshifted"] = (
        crosses_down
    )
